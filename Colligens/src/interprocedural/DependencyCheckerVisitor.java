...
public class DependencyCheckerVisitor implements Visitor{

	private FeatureExpr condExpr;
	private String idToFind;
	private String functionName;
	
	public String getIdToFind() {
		return idToFind;
	}

	public void setIdToFind(String idToFind) {
	...
	public void run(FunctionDef node) {
		if (node.getChildren().get(1).getChildren().get(0) instanceof Id){
			this.functionName = ((Id) node.getChildren().get(1).getChildren().get(0)).getName();
		}
		for (int i = 0; i < node.getChildren().size(); i++){
			node.getChildren().get(i).accept(this);
		}
	...
	public void run(Opt node) {
		this.condExpr = node.getConditional();
	...
	public void run(Id node) {
		...
		//if (this.condExpr != null && node.getName().equals(this.idToFind)){
		if (node.getPresenceCondition() != null && !node.getPresenceCondition().isTautology() && node.getName().equals(this.idToFind)){
			System.out.println("Feature " + this.condExpr.toTextExpr() + " in function (" + this.functionName +  ") uses " + this.idToFind + ".\n");
		}
