...
public class FunctionVariableVisitor implements Visitor{
		...
		if (atomicNamedDeclarator instanceof AtomicNamedDeclarator){
			...
			if (declParameterDeclList instanceof DeclParameterDeclList){
				for (int i = 0; i < declParameterDeclList.getChildren().size(); i++){
					...
					// Getting each function parameter
					Node functionParameter = declParameterDeclList.getChildren().get(i);
					if (functionParameter instanceof ParameterDeclarationD){
						Node atomicNamedDecl = functionParameter.getChildren().get(1);
						if (atomicNamedDecl instanceof AtomicNamedDeclarator){
							
							// it contains the name of the identifier (atomicNamedDecl)
							
							DependencyCheckerVisitor dependencyCheckerVisitor = new DependencyCheckerVisitor();
							
							if (atomicNamedDecl.getChildren().get(0) instanceof Id){
								dependencyCheckerVisitor.setIdToFind( ((Id)atomicNamedDecl.getChildren().get(0)).getName() );
								node.accept(dependencyCheckerVisitor);
							} else if (atomicNamedDecl.getChildren().get(0) instanceof Pointer){
								if (atomicNamedDecl.getChildren().get(1) instanceof Id){
									dependencyCheckerVisitor.setIdToFind( ((Id)atomicNamedDecl.getChildren().get(1)).getName() );
									node.accept(dependencyCheckerVisitor);
								} else if (atomicNamedDecl.getChildren().get(1) instanceof Pointer){
									if (atomicNamedDecl.getChildren().get(2) instanceof Id){
										dependencyCheckerVisitor.setIdToFind( ((Id)atomicNamedDecl.getChildren().get(2)).getName() );
										node.accept(dependencyCheckerVisitor);
									}
								}
							}
