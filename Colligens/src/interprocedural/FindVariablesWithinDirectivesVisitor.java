...
public class FindVariablesWithinDirectivesVisitor implements Visitor {

	private List<Id> variables = new ArrayList<>();

	public List<Id> getVariables() {
		return variables;
	}
	...
	public void run(Id node) {
		...
		if ((!(node.getParent() instanceof PostfixExpr)) && // function call
			(!(node.getParent() instanceof AtomicNamedDeclarator)) && // variable declaration
			(!(node.getParent().getParent() instanceof ParameterDeclarationD)) && // function parameter
			(!(node.getParent().getParent() instanceof FunctionDef))) { // function definition
