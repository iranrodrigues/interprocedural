...
public class FindFunctionParameterUsesVisitor implements Visitor {

	private Id variable;
	private List<Id> functionParameterUses = new ArrayList<Id>();
	
	public Id getVariable() {
		return variable;
	}
	
	public List<Id> getFunctionParameterUses() {
		return functionParameterUses;
	}

	public FindFunctionParameterUsesVisitor(Id variable) {
	...
	public void run(Id node) {
		if ((!((node.getParent().getChildren().size() > 1) && (node.getParent().getChildren().get(1) instanceof FunctionCall))) && // not a function call
			...
			(getVariable().getName().equals(node.getName()))) { // same name as function parameter
					this.functionParameterUses.add(node);
		}
		for (int i = 0; i < node.getChildren().size(); i++){
			node.getChildren().get(i).accept(this);
		}
