...
public class FindDirectivesVisitor implements Visitor {

	private List<Opt> preprocessorDirectives = new ArrayList<>();

	public List<Opt> getPreprocessorDirectives() {
	...
	public void run(Opt node) {
		if ((!node.getConditional().isTautology()) && (!node.getConditional().isContradiction())) {
			this.preprocessorDirectives.add(node);
		}
		for (int i = 0; i < node.getChildren().size(); i++){
			node.getChildren().get(i).accept(this);
		}
