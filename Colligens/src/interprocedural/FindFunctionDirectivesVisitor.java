...
public class FindFunctionDirectivesVisitor implements Visitor {

	private Set<Opt> functionDirectives = new HashSet<Opt>();

	public Set<Opt> getFunctionDirectives() {
	...
	public void run(FunctionDef node) {
		if (!(node.getPresenceCondition().isTautology())) {
			Opt o = new Opt();
			o.setPresenceCondition(node.getPresenceCondition());
			this.functionDirectives.add(o);
		}
		for (int i = 0; i < node.getChildren().size(); i++) {
			node.getChildren().get(i).accept(this);
		}
	...
	public void run(Opt node) {
		if (!(node.getConditional().isTautology())) {
			this.functionDirectives.add(node);
		}
