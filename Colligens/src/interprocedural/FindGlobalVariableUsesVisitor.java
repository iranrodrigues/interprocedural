...
public class FindGlobalVariableUsesVisitor implements Visitor {

	private Id variable;
	private List<Id> globalVariableUses = new ArrayList<Id>();
	
	public Id getVariable() {
		return variable;
	}
	
	public List<Id> getGlobalVariableUses() {
		return globalVariableUses;
	}

	public FindGlobalVariableUsesVisitor(Id variable) {
	...
	public void run(Id node) {
		...
		if ((!((node.getParent().getChildren().size() > 1) && (node.getParent().getChildren().get(1) instanceof FunctionCall))) && // not a function call
			...
			(!(node.getPresenceCondition().and(this.getVariable().getPresenceCondition()).isContradiction())))) { // presence condition in declaration *and* use cannot be a contradiction
					this.globalVariableUses.add(node);
		}
		for (int i = 0; i < node.getChildren().size(); i++){
			node.getChildren().get(i).accept(this);
		}
